...
3. The exact corresponding-source archive generated by the pinned FFmpeg builder, including FFmpeg, every statically linked non-system library, the Docker recipe, toolchain package inventory, and configure command, must be attached to the same release.
